...
__global__ void AliasSingleSampleKernel(int *output,
                                  // LocalIdType *src_lid,
                                  MAT_PTR_TYPE *sample_offset,
                                  int *input_nodes,
                                  int input_node_count,
                                  ...
                                  MAT_VAL_TYPE *prob)
{
  ...
  if (neighbor_count <= max_sample_count)
  {
    ...
    //   output[offset + sample_id] = gid;
    // }
    for (int sample_id = 0; sample_id < neighbor_count && sample_id < max_sample_count; sample_id ++)
    {
      int neighbor_idx = sample_id;
      int gid = columnindexA[start + neighbor_idx];
      output[offset + sample_id] = gid;
...
*/
  ...
  for(int iter = 0; iter < 1000; iter++)
  {
    ...
    cudaDeviceSynchronize();
    gettimeofday(&t1, NULL);
    int block_num = sample_size / 64 + 1;
    AliasSingleSampleKernel<<<block_num, 64>>>(d_sample_output,
                                         // src_lid,
                                         d_sample_offset,
                                         d_input_nodes,
                                         sample_size,
...
//------------------------------l2-------------------------------
  ...
  for(int iter = 0; iter < 1000; iter++)
  {
    ...
    cudaDeviceSynchronize();
    gettimeofday(&t1, NULL);
    int block_num = sample_size_l2 / 64 + 1;
    AliasSingleSampleKernel<<<block_num, 64>>>(d_sample_output_l2,
                                         // src_lid,
                                         d_sample_offset_l2,
                                         d_sample_output,
                                         sample_size_l2,
...
FILE *fout = fopen("sage-4090-25-10.csv", "a");
    if (fout == NULL)
