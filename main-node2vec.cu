  ...
  float q)
{
  ...
  for(int dep=0;dep<length;dep++) {
      ...
      int t;//前一个采样点
      if(dep==0) t=input_idx;
      else t=output[offset+dep-1]; 
      ...
      if(neighbor_count==1) {//只有一条边
        ...
        for(int i=start+tidx;i<end;i+=32) {
          ...
          bool flag=false;
          int l=rowpointerA[t];
          int r=rowpointerA[t+1]-1;

          while(l<=r&&columnindexA[l]<=cur&&columnindexA[r]>=cur) {
            int mid=(l+r)/2;
            if(columnindexA[mid]==cur) {
              flag=true;
              break;
            }
            if(columnindexA[mid]>cur) {
...
void construct_aliastable(int *alias,double *prob,double *b,Matrix *matirx,int *pre_edge) {
  for(int i=0;i<matrix->nnz;i++) {//第i条边
    ...
    while(!large.empty()&&!small.empty()) {
      ...
      int cur=matrix->columnidx[matrix->rowpointer[x]+vl];//x的第vl个相邻节点
      alias[pre+vs]=cur;
...
int main(int argc, char **argv)
{
  ...
  construct_aliastable(alias,prob,b,matrixA,pre_edge);
  ...
  for(int iter = 0; iter < 1000; iter++)
  {
    ...
    Node2vecKernel<<<sample_size/4 + 1, 128>>>(d_sample_output,//采样结果
                                         // src_lid,
