...
__global__ void DeepwalkKernel(int *output,//采样结果
  MAT_PTR_TYPE *sample_offset,//采样个数前缀和
  int *input_nodes,//sample node
  int input_node_count,
  MAT_PTR_TYPE *rowpointerA,
...
*/
  ...
  {
    DeepwalkKernel<<<sample_size, 64>>>(d_sample_output,//采样结果
                                         // src_lid,
...
FILE *fout = fopen("deepwalk-219.csv", "a");
    if (fout == NULL)
        printf("Writing results fails.\n");
    fprintf(fout, "%s,%i,%i,%i,%f,%f\n",
            filename,  matrixA_d->m, matrixA_d->n, matrixA_d->nnz,time_alias_table_constrcut,ave_time_alias_table_sample);
    fclose(fout);
//-------------------------------l2 end--------------------------------
