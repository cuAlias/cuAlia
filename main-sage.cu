    ...
    int nnz)
{
  ...
  int input_idx = gidx / 32;//warp id
  int lid = gidx % 32;//warp 里面 thread id
  ...
  for (int i = lid; i < neighbor_count; i += 32)//一个warp中的每个线程处理gap=32的邻居
  {
  ...
  if (weight_sum != 0.0)
  {
    ...
    for (int i = lid; i < neighbor_count; i += 32)
    {
      ...
      p[start + i] = value[start + i] * scale;//p = count * val / sum
    }
  }
  else
  {
  ...
  small_idx_ptr[input_idx] = 0;//p<1的个数
  large_idx_ptr[input_idx] = 0;//p>=1的个数
  for (int i = lid; i < neighbor_count; i += 32)
  {
    prob[start + i] = 1.0;
    alias[start + i] = 0;
  ...
  for (int i = lid; i < neighbor_count; i += 32)
  {
    ...
    if (tmp_weight >= 1)
    {
      ...
      large[start + k] = i;//p>1的边id
    }
    else
    {
      int k = atomicAdd(&small_idx_ptr[input_idx], 1);
      small[start + k] = i;//p<=1的边id
    }
  }
...
__global__ void AliasSampleKernel(int *output,//采样结果
                                  // LocalIdType *src_lid,
                                  ...
                                  MAT_VAL_TYPE *prob)
{
  ...
  // int tidx = gidx%32;
  int input_idx = blockIdx.x;//采样起始点
...
*/
  ...
  for(int iter = 0; iter < 1000; iter++)
  {
    ...
    //sample_size 个block，每个block2个warp
    AliasSampleKernel<<<sample_size, 64>>>(d_sample_output,//采样结果
                                         // src_lid,
...
//------------------------------l2-------------------------------
  ...
  int sample_count_l2 = 10; // 单个点采样数量
  int sample_size_l2 = sample_offset_sum;
  MAT_PTR_TYPE *sample_offset_l2 = (MAT_PTR_TYPE *)malloc(sizeof(MAT_PTR_TYPE) * (sample_size_l2 + 1));
  for (int i = 0; i < sample_size_l2; i++)
  {
    MAT_PTR_TYPE vid = sample_output[i];
    int neighbor_count = (int)(matrixA_d->rowpointer[vid + 1] - matrixA_d->rowpointer[vid]);
    sample_offset_l2[i] = min(neighbor_count, sample_count_l2);
  }
  exclusive_scan(sample_offset_l2, sample_size_l2 + 1);
  MAT_PTR_TYPE *d_sample_offset_l2;
  cudaMalloc((void **)&d_sample_offset_l2, sizeof(MAT_PTR_TYPE) * (sample_size_l2 + 1));
  cudaMemcpy(d_sample_offset_l2, sample_offset_l2, sizeof(MAT_PTR_TYPE) * (sample_size_l2 + 1), cudaMemcpyHostToDevice);
  MAT_PTR_TYPE sample_offset_sum_l2 = sample_offset_l2[sample_size_l2]; // 所有点实际采样数量

  int *d_sample_output_l2;
  cudaMalloc((void **)&d_sample_output_l2, sizeof(int) * sample_offset_sum_l2);

  double ave_time_alias_table_sample_l2 = 0;
  for(int iter = 0; iter < 1000; iter++)
  {
    cudaMemset(d_sample_output_l2, 0, sizeof(int) * sample_offset_sum_l2);
    cudaDeviceSynchronize();
    gettimeofday(&t1, NULL);

    AliasSampleKernel<<<sample_size_l2, 64>>>(d_sample_output_l2,
                                         // src_lid,
                                         d_sample_offset_l2,
                                         d_sample_output,
                                         sample_size_l2,
    ...
    gettimeofday(&t2, NULL);
    double time_alias_table_sample = (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
    ave_time_alias_table_sample_l2 += time_alias_table_sample;
  }
  ave_time_alias_table_sample_l2=ave_time_alias_table_sample_l2/1000;
  
  printf("Alias Table Sample L2: %f ms\n", ave_time_alias_table_sample_l2);


FILE *fout = fopen("sage-4090-25-10-217.csv", "a");
    if (fout == NULL)
        printf("Writing results fails.\n");
    fprintf(fout, "%s,%i,%i,%i,%f,%f,%f,%f\n",
            filename,  matrixA_d->m, matrixA_d->n, matrixA_d->nnz,time_alias_table_constrcut,ave_time_alias_table_sample, ave_time_alias_table_sample_l2,
            ave_time_alias_table_sample_l2+ave_time_alias_table_sample);
    fclose(fout);
//-------------------------------l2 end--------------------------------
